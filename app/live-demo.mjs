...
const VIEM_CHAINS_URL = 'https://esm.sh/viem@2.21.54/chains';
...
export async function initLiveDemo({ ui }) {
  const [{ createPublicClient, createWalletClient, custom, http, parseUnits, formatUnits, keccak256, encodeAbiParameters, stringToHex, decodeEventLog, isAddress }, { defineChain }] = await Promise.all([import(VIEM_URL), import(VIEM_CHAINS_URL)]);
